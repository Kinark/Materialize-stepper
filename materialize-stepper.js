...
$.fn.openStep = function (step, callback) {
    ...
    step.openAction(order, function () {
        ...
        if ($this.closest('ul').hasClass('horizontal')) {
            ...
            $('.stepper.horizontal').find('.step.active').find('.step-content > div').each(function (index, value) {
                console.log(value)
                newHeight = newHeight + parseInt($(value).css('height'));
            });

            newHeight += padding;

            $('.stepper.horizontal').animate({
                height: newHeight
            },
                300);
